...
#define ID_WINDOW_PICTURE    (GUI_ID_USER + 0x00)
...
WM_HWIN hwin_disconnect = WM_HWIN_NULL;
...
{
    { WINDOW_CreateIndirect, "wm_picture",   ID_WINDOW_PICTURE,   0, 0, 128, 64, 0, 0x0, 0 },
...
static void wm_disconnect_init(const WM_MESSAGE * pMsg)
{
    htimer_battery = WM_CreateTimer(WM_GetClientWindow(pMsg->hWin), ID_TIMER_BATTERY, 200, 0);
    IMAGE_SetBitmap(WM_GetDialogItem(pMsg->hWin, ID_IMAGE_STATUS),&bmdisconnect_128x49);
...
static void wm_disconnect_battery_image_doing(const WM_MESSAGE * pMsg)
{
...
static void wm_disconnect_htimer(const WM_MESSAGE * pMsg)
{
...
static void wm_disconnect_update_battery(const WM_MESSAGE * pMsg)
{
...
static void wm_disconnect_free_window_object(const WM_MESSAGE * pMsg)
{
    ...
    WM_DeleteWindow(WM_GetDialogItem(pMsg->hWin, ID_WINDOW_PICTURE));
}
...
static void _cbDialog(WM_MESSAGE * pMsg) 
{
    ...
    {
        case WM_INIT_DIALOG:
            wm_disconnect_init(pMsg);
            break;
        case WM_DELETE:
            wm_disconnect_free_window_object(pMsg);
            break;
        case WM_TIMER:
            ...
            switch (id)
            {
                case ID_TIMER_BATTERY:
                    wm_disconnect_update_battery(pMsg);
                    WM_RestartTimer(pMsg->Data.v, 200);
                    break;
                case ID_TIMER_BATTERY_RUNING:
                    wm_disconnect_battery_image_doing(pMsg);
...
void create_disconnect_window(void) 
{
    hwin_disconnect = GUI_CreateDialogBox(_aDialogCreate, GUI_COUNTOF(_aDialogCreate), _cbDialog, WM_HBKWIN, 0, 0);
}
